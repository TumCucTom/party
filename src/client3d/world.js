...
export class World {
  ...
  constructor({ seed, scene, materials, viewRadius = 7, smoothLighting = true }) {
    ...
    // FROM the server go through applyRemoteEdit() and never re-fire it.
    this.onEdit = null;
  ...
  setBlock(x, y, z, id, { record = true, silent = false } = {}) {
    ...
    if (record) {
      ...
      m.set(i, id);
      if (this.onEdit && !this._applyingRemote) this.onEdit(x, y, z, id);
    }
  ...
  /** Load edits sent by the server as [x, y, z, id] world coordinates. */
  loadWorldEdits(list) {
    if (!Array.isArray(list)) return;
    for (const [x, y, z, id] of list) this._recordEdit(x, y, z, id);
  }

  _recordEdit(x, y, z, id) {
    const key = ckey(Math.floor(x / CHUNK), Math.floor(z / CHUNK));
    let m = this.editsByChunk.get(key);
    if (!m) { m = new Map(); this.editsByChunk.set(key, m); }
  ...
  /**
   * Apply an edit broadcast by another player. Applied silently (the
   * originating client simulates support/gravity cascades and sends the
   * resulting edits itself) and without re-firing onEdit. If the chunk
   * isn't loaded yet, the edit is recorded and applied on generation.
   */
  applyRemoteEdit(x, y, z, id) {
    ...
    if (c && c.blocks) {
      ...
      try {
        this.setBlock(x, y, z, id, { record: true, silent: true });
      } finally {
        this._applyingRemote = false;
      }
    } else {
